  ...
  <l-map class="map" ref="map" :min-zoom="minZoom" :crs="crs">
    ...
    <l-layer-group>
      <l-marker v-for="(newCoords, i) in InvertedCoords" :key="i" :lat-lng="newCoords">
        <!-- <l-tooltip :content="stars[i].name" :options="options"></l-tooltip> -->
        ...
        <l-popup class="popup">
          <em class="popup-bold">Id:</em>
          {{ newCoords.id }}
          <br />
          <!-- Lat: {{ newCoords.lat }}, Lng: {{ newCoords.lng }},<br> -->
          <em class="popup-bold">Name:</em>
          {{ stars[i].name }}
          <br />
          <em class="popup-bold">Longitud:</em>
          {{ stars[i].lng }}
          <br />
          <em class="popup-bold">Latitud:</em>
          {{ stars[i].lat }}
          <br />
          <em class="popup-bold">Number of Planets:</em>
          {{ stars[i].planetCount }}
          <br />
          <em class="popup-bold">Local Population 1:</em>
          {{ stars[i].localPop1 }}
          <br />
          <em class="popup-bold">Planet 1:</em>
          {{ stars[i].planet1 }}
          <br />
          <em class="popup-bold">Local Population 2:</em>
          {{ stars[i].localPop2 }}
          <br />
          <em class="popup-bold">Planet 2:</em>
          {{ stars[i].planet2 }}
          <br />
          <em class="popup-bold">Local Population 3:</em>
          {{ stars[i].localPop3 }}
          <br />
          <em class="popup-bold">Planet 3:</em>
          {{ stars[i].planet3 }}
          <br />
          <em class="popup-bold">System Owner:</em>
          {{ stars[i].systemOwner }}
          <br />
          <em class="popup-bold">Radar:</em>
          {{ stars[i].radar }}
          <br />
          <em class="popup-bold">Comments:</em>
          {{ stars[i].comments }}
          <br />
          <em class="popup-bold">Added by:</em>
          {{ stars[i].addedBy }}
          <br />
          <em class="popup-bold">Date:</em>
          {{ stars[i].date }}
          <br />
          <em class="popup-bold">Status:</em>
          {{ stars[i].status }}
          <br />
          <!-- TEST: {{ InvertedCoords }} -->
        </l-popup>
    ...
    <div v-for="(newCoords, i) in InvertedCoords" :key="'A' + i">
      <!-- 'A' + i to avoid duplicate id -->
      <l-circle
        v-if="stars[i].radar === '1'"
        :lat-lng="newCoords"
...
export default {
  ...
  created() {
    ...
    axios
      ...
      .then((response) => {
        ...
        // console.log(resultArray) // testing
        this.stars = resultArray;
      })
    ...
    //   console.log('Current Zoom: ' + currentZoom);
      // return L.map.getZoom === this.currentZoom;
    ...
    // L.map.getZoom() === this.currentZoom;
  },
  ...
  methods: {
    ...
    //   var currentZoom = map.getZoom();
    //   console.log('Current Zoom: ' + currentZoom);
    //   return currentZoom === this.currentZoom;
    // }
  },
};
</script>
